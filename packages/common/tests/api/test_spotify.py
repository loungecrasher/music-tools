...
class TestSpotifyClientRequests:
    ...
    def test_get_track_success(self, mock_get):
        ...
        mock_get.assert_called_once_with(
            "https://api.spotify.com/v1/tracks/3n3Ppam7vgaVa1iaRUc9Lp", params=None
        )
    ...
    def test_search_tracks(self, mock_get):
        ...
        mock_get.assert_called_once_with("https://api.spotify.com/v1/search", params=params)
...
class TestSpotifyClientDataRetrieval:
    ...
    def test_get_new_releases(self, mock_get):
        ...
        mock_response.json.return_value = {
            "albums": {
                "items": [{"id": "1", "name": "New Album 1"}, {"id": "2", "name": "New Album 2"}]
            }
        }
        mock_get.return_value = mock_response
    ...
    def test_get_categories(self, mock_get):
        ...
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "categories": {"items": [{"id": "rock", "name": "Rock"}, {"id": "pop", "name": "Pop"}]}
        }
        mock_get.return_value = mock_response
    ...
    def test_get_recommendations(self, mock_get):
        ...
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "tracks": [{"id": "1", "name": "Recommended 1"}, {"id": "2", "name": "Recommended 2"}],
            "seeds": [{"id": "seed1", "type": "artist"}],
        }
        mock_get.return_value = mock_response

        client = SpotifyClient()
        params = {"seed_artists": "artist1,artist2", "limit": 2}
...
class TestSpotifyClientEdgeCases:
    ...
    def test_empty_search_results(self, mock_get):
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"tracks": {"items": [], "total": 0}}
        mock_get.return_value = mock_response
    ...
    def test_unicode_in_search(self, mock_get):
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"tracks": {"items": [{"name": "Björk - Army of Me"}]}}
        mock_get.return_value = mock_response
...
class TestSpotifyClientIntegration:
    ...
    def test_search_and_get_details_workflow(self, mock_get):
        ...
        search_response.json.return_value = {
            "tracks": {"items": [{"id": "track123", "name": "Test Track"}]}
        }
