...
def all_clients(spotify_client, deezer_client):
    ...
    return {"spotify": spotify_client, "deezer": deezer_client}
...
def sample_spotify_search_results():
    ...
    return {
        "tracks": {
            "items": [
                {"id": "track1", "name": "Rock Song 1", "artists": [{"name": "Artist 1"}]},
                {"id": "track2", "name": "Rock Song 2", "artists": [{"name": "Artist 2"}]},
            ],
            "total": 100,
            "limit": 2,
            "offset": 0,
...
def sample_deezer_search_results():
    ...
    return {
        "data": [
            {"id": 1, "title": "Rock Song 1", "artist": {"name": "Artist 1"}},
            {"id": 2, "title": "Rock Song 2", "artist": {"name": "Artist 2"}},
        ],
        "total": 100,
    }
...
def pytest_generate_tests(metafunc):
    ...
    if "spotify_endpoint" in metafunc.fixturenames:
        endpoints = ["tracks/123", "artists/456", "albums/789", "playlists/abc", "search"]
        metafunc.parametrize("spotify_endpoint", endpoints)

    if "deezer_endpoint" in metafunc.fixturenames:
        endpoints = ["track/123", "artist/456", "album/789", "playlist/abc", "search"]
