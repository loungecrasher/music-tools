...
class TestLibraryFile:
    ...
    def test_filename_derived_from_path(self):
        f = LibraryFile(file_path='/music/my_song.flac', filename='', file_format='flac',
                        file_size=100, metadata_hash='h', file_content_hash='c')
        assert f.filename == 'my_song.flac'

    def test_format_derived_from_path(self):
        f = LibraryFile(file_path='/music/track.m4a', filename='track.m4a', file_format='',
                        file_size=100, metadata_hash='h', file_content_hash='c')
...
class TestDuplicateResult:
    ...
    def test_creation_exact_match(self):
        ...
        r = DuplicateResult(
            is_duplicate=True, confidence=1.0,
            match_type='exact_metadata', matched_file=matched
        )
...
class TestVettingReport:
    ...
    def test_new_count(self):
        r = make_vetting_report(new_songs=['/import/new1.mp3', '/import/new2.mp3', '/import/new3.mp3'])
    ...
    def test_duplicate_percentage(self):
        ...
        r = make_vetting_report(
            total_files=100,
            duplicates=[('/import/dup.mp3', dup_result)] * 25
        )
    ...
    def test_new_percentage(self):
        r = make_vetting_report(total_files=200, new_songs=[f'/import/new{i}.mp3' for i in range(180)])
...
class TestLibraryStatistics:
    ...
    def test_get_format_percentages(self):
        s = LibraryStatistics(
            total_files=100,
            formats_breakdown={'mp3': 60, 'flac': 40}
        )
