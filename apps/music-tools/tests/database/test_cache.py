...
class TestCacheManager:
    ...
    def test_hit_count_tracking(self, cache):
        ...
        result = cache.fetch_one("SELECT hit_count FROM cache_entries WHERE key = ?", ("key1",))
    ...
    def test_metadata_storage(self, cache):
        ...
        metadata = {"source": "API", "confidence": 0.95, "timestamp": datetime.now().isoformat()}

        cache.set("key1", "value1", metadata=metadata)

        # Verify metadata is stored
        result = cache.fetch_one("SELECT metadata FROM cache_entries WHERE key = ?", ("key1",))
    ...
    def test_custom_ttl(self, cache):
        ...
        result1 = cache.fetch_one("SELECT expires_at FROM cache_entries WHERE key = ?", ("key1",))
        result2 = cache.fetch_one("SELECT expires_at FROM cache_entries WHERE key = ?", ("key2",))
    ...
    def test_cleanup_old_entries(self, temp_cache_dir):
        ...
        cache.execute(
            "UPDATE cache_entries SET created_at = ? WHERE key = ?", (old_date, "key1"), commit=True
        )
