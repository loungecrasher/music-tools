    ...
    def filter_posts_by_genre(self, post_urls: List[str], target_genres: List[str] = None,
        ...
        with tqdm(total=len(post_urls), desc="Filtering preferred genres", unit="post") as pbar:
            for i, post_url in enumerate(post_urls, 1):
                ...
                if start_date or end_date:
                    if not post_date:
                        logger.debug(f"Could not determine post date, skipping date filter")
                ...
                if matching_genres:
                    ...
                    logger.debug(f"No matching genres")
...
def main():
    ...
    if args.start_date:
        try:
            ...
            logger.error(f"Invalid start date format. Use YYYY-MM-DD (e.g., 2024-01-15)")
            return
    ...
    if args.end_date:
        try:
            ...
            logger.error(f"Invalid end date format. Use YYYY-MM-DD (e.g., 2024-12-31)")
            return
