...
from .config import (
    DOWNLOAD_PATTERNS, RELEASE_IDENTIFIER_PATTERNS, DATE_SELECTORS, 
    META_DATE_SELECTORS, DATE_PATTERNS, URL_DATE_PATTERNS, DATE_FORMATS,
    BLOG_POST_SELECTORS, GENRE_SELECTORS, TITLE_SELECTORS, 
    PAGINATION_PATTERNS, SKIP_URL_PATTERNS, POST_URL_INDICATORS,
    REQUEST_TIMEOUT, REQUEST_DELAY, DEFAULT_MAX_PAGES, MAX_PAGES_LIMIT,
    MAX_EMPTY_PAGES, POSTS_PER_PAGE_ESTIMATE, DEFAULT_OUTPUT_FILE,
    DEFAULT_ENCODING, MAX_URL_LENGTH, MAX_TITLE_LENGTH, MAX_GENRE_LENGTH,
    VALID_HOSTS, USER_AGENTS, GROUP_SIZE, DEFAULT_GENRES, ALL_EDM_GENRES
)
...
class MusicBlogScraper:
    ...
    def find_blog_posts(self, max_pages: int = DEFAULT_MAX_PAGES, start_date=None, end_date=None) -> List[str]:
        ...
        if main_soup:
            ...
            logger.error(f"Could not access main page")
            return []
        ...
        with tqdm(total=len(post_urls), desc="Filtering posts by genre", unit="post") as pbar:
            for i, post_url in enumerate(post_urls, 1):
                ...
                if matching_genres:
                    ...
                    logger.debug(f"No matching genres found")
...
def main():
    ...
    if args.start_date:
        try:
            ...
            print(f"Error: Invalid start date format. Use YYYY-MM-DD (e.g., 2024-01-15)")
            return 1
    ...
    if args.end_date:
        try:
            ...
            print(f"Error: Invalid end date format. Use YYYY-MM-DD (e.g., 2024-12-31)")
            return 1
