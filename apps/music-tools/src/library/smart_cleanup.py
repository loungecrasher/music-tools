...
class SmartCleanupWorkflow:
    ...
    def _show_welcome(self) -> bool:
        ...
        welcome_text.append("Safely identify and remove duplicate music files while ", style="white")
        ...
        if stats.formats_breakdown:
            welcome_text.append("\n  Format Breakdown:\n", style="bold")
            for fmt, count in sorted(stats.formats_breakdown.items(), key=lambda x: x[1], reverse=True)[:5]:
                percentage = (count / stats.total_files * 100) if stats.total_files > 0 else 0
                welcome_text.append(f"    {fmt.upper()}: {count:,} ({percentage:.1f}%)\n", style="white")
        ...
        panel = Panel(welcome_text, title="Welcome to Smart Cleanup", border_style="cyan", box=box.DOUBLE)
        self.console.print(panel)
    ...
    def _configure_custom_mode(self) -> ScanMode:
        ...
        fuzzy_threshold = float(Prompt.ask(
    ...
    def scan_for_duplicates(self, mode: ScanMode, path: Path) -> None:
        ...
        with progress:
            ...
            task2 = progress.add_task(
                ...
                total=len(duplicate_hash_groups)
            )
    ...
    def _execute_cleanup(self) -> None:
        ...
        with Progress(
            ...
            # Phase 1: Backup
            task1 = progress.add_task(
                "[cyan]Creating backups...",
                total=self.stats.files_to_delete
            )

            # Phase 2: Delete
            task2 = progress.add_task(
                "[yellow]Deleting files...",
                total=self.stats.files_to_delete
            )

            # Execute deletion plan
            deletion_stats = self.deletion_plan.execute(
                dry_run=False,
                create_backup=True
            )
    ...
    def show_completion_summary(self, stats: CleanupStats) -> None:
        ...
        summary_text.append(f"  Duplicate Groups: {stats.duplicate_groups_found:,}\n", style="white")
        ...
        panel = Panel(summary_text, title="Completion Summary", border_style="green", box=box.DOUBLE)
        self.console.print("\n", panel)
    ...
    def _export_reports(self) -> None:
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                'Group ID', 'Action', 'File Path', 'Format', 'Quality Score',
                'File Size MB', 'Bitrate Type', 'Sample Rate'
            ])
            ...
            for group in self.duplicate_groups:
                if group.user_confirmed:
                    # Keep file
                    writer.writerow([
                        group.group_id, 'KEEP', group.recommended_keep.filepath,
                        group.recommended_keep.format, group.recommended_keep.quality_score,
                    ...
                    for file in group.recommended_delete:
                        writer.writerow([
                            group.group_id, 'DELETE', file.filepath,
                            file.format, file.quality_score,
