...
npx claude-flow@alpha swarm init --topology hierarchical-mesh --max-agents 15 --strategy specialized
```
...
Bash("npx claude-flow@alpha swarm init --topology hierarchical --max-agents 8 --strategy specialized")
...
Bash("npx claude-flow@alpha memory search --query '[task keywords]' --namespace patterns")

# 2. Check if similar task was done before
Bash("npx claude-flow@alpha memory search --query '[task type]' --namespace tasks")

# 3. Load learned optimizations
Bash("npx claude-flow@alpha hooks route --task '[task description]'")
```

### After Completing Any Task Successfully
```bash
# 1. Store successful pattern for future reference
Bash("npx claude-flow@alpha memory store --namespace patterns --key '[pattern-name]' --value '[what worked]'")

# 2. Train neural patterns on the successful approach
Bash("npx claude-flow@alpha hooks post-edit --file '[main-file]' --train-neural true")

# 3. Record task completion with metrics
Bash("npx claude-flow@alpha hooks post-task --task-id '[id]' --success true --store-results true")

# 4. Trigger optimization worker if performance-related
Bash("npx claude-flow@alpha hooks worker dispatch --trigger optimize")
```
...
## � KODI MANAGER CLI SKILLS

**Available domain-specific skills in `.claude/skills/`:**

| Skill | Purpose | When to Reference |
|-------|---------|-------------------|
| `kodi-bug-fix.md` | Bug patterns, debugging workflows, known fixes | Debugging issues, fixing bugs, investigating errors |
| `kodi-feature.md` | Feature implementation following architectural patterns | Adding new features, extending functionality |
| `kodi-scheduler.md` | Scheduler subsystem development (cron, threading, events) | Working with scheduled tasks, daemon operations |
| `kodi-alert.md` | Alert system development (multi-channel alerting) | Adding alerts, modifying notification channels |
| `kodi-security.md` | Security standards (RBAC, input validation, secrets) | Security-related changes, authentication work |
| `kodi-fleet.md` | Fleet management operations (13 production servers) | Fleet operations, deployment, server management |

**How to use:**
1. **Before starting work**: Read the relevant skill to understand domain patterns
2. **During implementation**: Follow architectural patterns from the skill
3. **After completion**: Verify against skill checklists and best practices

**Quick reference**: See `.claude/skills/README.md` for detailed skill descriptions.
...
npx claude-flow@alpha daemon start

# Spawn an agent
npx claude-flow@alpha agent spawn -t coder --name my-coder

# Initialize swarm
npx claude-flow@alpha swarm init --v3-mode

# Search memory (HNSW-indexed)
npx claude-flow@alpha memory search --query "authentication patterns"

# System diagnostics
npx claude-flow@alpha doctor --fix

# Security scan
npx claude-flow@alpha security scan --depth full

# Performance benchmark
npx claude-flow@alpha performance benchmark --suite all
```
...
npx claude-flow@alpha hooks post-edit --file "[file]" --train-neural true
...
npx claude-flow@alpha hooks route --task "[task]"
npx claude-flow@alpha hooks explain --topic "[topic]"

# Neural learning
npx claude-flow@alpha hooks pretrain --model-type moe --epochs 10
npx claude-flow@alpha hooks build-agents --agent-types coder,tester
...
npx claude-flow@alpha hooks coverage-gaps --format table
npx claude-flow@alpha hooks coverage-route --task "[task]"

# Statusline (for Claude Code integration)
npx claude-flow@alpha hooks statusline
npx claude-flow@alpha hooks statusline --json
```
...
npx claude-flow@alpha migrate status

# Run migration with backup
npx claude-flow@alpha migrate run --backup

# Rollback if needed
npx claude-flow@alpha migrate rollback

# Validate migration
npx claude-flow@alpha migrate validate
```
...
Bash("npx claude-flow@alpha hooks post-command --command '[operation]' --track-metrics true")

# Periodically run benchmarks (every major feature)
Bash("npx claude-flow@alpha performance benchmark --suite all")

# Analyze bottlenecks when performance degrades
Bash("npx claude-flow@alpha performance profile --target '[component]'")
```

### Session Persistence (Cross-Conversation Learning)
```bash
# At session start - restore previous context
Bash("npx claude-flow@alpha session restore --latest")

# At session end - persist learned patterns
Bash("npx claude-flow@alpha hooks session-end --generate-summary true --persist-state true --export-metrics true")
```

### Neural Pattern Training
```bash
# Train on successful code patterns
Bash("npx claude-flow@alpha neural train --pattern-type coordination --epochs 10")

# Predict optimal approach for new tasks
Bash("npx claude-flow@alpha neural predict --input '[task description]'")

# View learned patterns
Bash("npx claude-flow@alpha neural patterns --list")
```

## � Contract & Dependency Pipeline

**On every change, the system automatically:**
1. Generates a **module-level contract summary**.
2. Updates the **dependency graph**.
3. **Feeds ONLY the touched contracts** back to the agent for context efficiency.

Usage:
```bash
# This pipeline runs automatically via hooks, but can be triggered manually:
npx claude-flow@alpha hooks pipeline contracts --optimize-context true
```
...
Run `npx claude-flow@alpha doctor` to check:
- Node.js version (20+)
- npm version (9+)
- Git installation
...
claude mcp add claude-flow -- npx -y claude-flow@alpha
...
- **Swarm init**: `npx claude-flow@alpha swarm init --topology <type>`
- **Swarm status**: `npx claude-flow@alpha swarm status`
- **Agent spawn**: `npx claude-flow@alpha agent spawn -t <type> --name <name>`
- **Memory store**: `npx claude-flow@alpha memory store --key "mykey" --value "myvalue" --namespace patterns`
- **Memory search**: `npx claude-flow@alpha memory search --query "search terms"`
- **Memory list**: `npx claude-flow@alpha memory list --namespace patterns`
- **Memory retrieve**: `npx claude-flow@alpha memory retrieve --key "mykey" --namespace patterns`
- **Hooks**: `npx claude-flow@alpha hooks <hook-name> [options]`
...
npx claude-flow@alpha memory store --key "pattern-auth" --value "JWT with refresh tokens" --namespace patterns
npx claude-flow@alpha memory store --key "bug-fix-123" --value "Fixed null check" --namespace solutions --tags "bugfix,auth"
```
...
npx claude-flow@alpha memory search --query "authentication patterns"
npx claude-flow@alpha memory search --query "error handling" --namespace patterns --limit 5
```
...
npx claude-flow@alpha memory list --namespace patterns --limit 10
```
...
npx claude-flow@alpha memory retrieve --key "pattern-auth"
npx claude-flow@alpha memory retrieve --key "pattern-auth" --namespace patterns
```

### Initialize Memory Database
```bash
npx claude-flow@alpha memory init --force --verbose
```
